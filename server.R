...
function(input, output){
  ...
  output$den2 <- renderPlot({
    ...
    hist(iris$Sepal.Length, breaks = 12, prob = TRUE, xlab = "Sepal Length", main = "Sepal Length Histogram / Density")
    lines(d2, lty = 3, col= "green")
  ...
  output$den3 <- renderPlot({
    ...
    abline(v=NP()$Petal.Width, col = "red")
  })
  output$den4 <- renderPlot({
    d4 <- density(iris$Petal.Length)
    hist(iris$Petal.Length, breaks = 12, prob = TRUE, xlab = "Petal Length", main = "Petal Length Histogram / Density")
